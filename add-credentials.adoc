...
Saiba mais sobre as permissões necessárias para cada funcionalidade de carga de trabalho em link:permissions-reference.html["Referência de permissões"].
...
. Na página Adicionar credenciais, selecione *Adicionar manualmente* e siga as etapas a seguir para preencher cada seção em _Configuração de permissões_.
+
...
NOTE: Se você tiver uma conta GovCloud, não precisará das permissões do Cost Explorer `ce:GetCostAndUsage` ou `ce:GetTags` na política **Visualizar, planejar e analisar** para Storage. Depois de copiar o script da caixa de código, remova essas permissões e apague a vírgula final da linha anterior.
...
.Passos
. Na página Adicionar Credenciais do Workload Factory, expanda *Criar função* em *Configuração de permissões*.
. Selecione *Código JSON* para copiar a política.
...
. Substitua o conteúdo atual na caixa de texto pelo código JSON que você copiou na etapa 2.
. Selecione *seguinte*.
...
. Copie a função ARN.
. Retorne à página Adicionar Credenciais no Workload Factory e cole o ARN da função no campo _Role ARN_.
...
NOTE: Adicionar credenciais a uma conta da AWS GovCloud (EUA) usando CloudFormation não é compatível.
