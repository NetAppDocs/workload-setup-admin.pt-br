...
| A implantação do servidor Microsoft SQL Server/PostgreSQL foi bem-sucedida | A implantação do host Microsoft SQL Server ou PostgreSQL foi bem-sucedida. Para mais informações, acesse monitoramento de tarefas. | Implantação | Sucesso | Bancos de dados | FSx para ONTAP, host de banco de dados 
