...
---
= Gerencie contas de serviço no NetApp Workload Factory
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Opcionalmente, você pode obter um token de acesso para a API de gerenciamento do Auth0 executando uma troca de credenciais do cliente. Consulte link:https://docs.netapp.com/us-en/console-automation/platform/grant_types.html#refresh-token["NetApp Console Documentação da API"^] para obter informações sobre como gerar um token de atualização.
+
O exemplo com curl mostra como você pode pegar o client ID e o secret e usar uma API para gerar o token de acesso, que é limitado por tempo. O token fornece várias horas de acesso às APIs do NetApp Workload Factory.
...
----
+
Poste a saída em https://netapp-cloud-account.auth0.com/oauth/token para obter o token de acesso.
