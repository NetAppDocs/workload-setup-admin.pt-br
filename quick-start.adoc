...
O NetApp Workload Factory está acessível aos usuários como um serviço em nuvem a partir do console baseado na web.  Antes de começar, você deve ter uma compreensão delink:workload-factory-overview.html["Fábrica de Carga de Trabalho"] .
...
[role="quick-margin-para"]
Esta etapa é opcional. Você pode usar o Workload Factory sem precisar adicionar credenciais para acessar sua conta da AWS.  Adicionar credenciais da AWS ao Workload Factory concede à sua conta do Workload Factory as permissões necessárias para criar e gerenciar sistemas de arquivos FSx para ONTAP e para implantar e gerenciar cargas de trabalho específicas, como bancos de dados e GenAI.
