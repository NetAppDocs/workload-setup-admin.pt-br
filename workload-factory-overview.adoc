...
Uma _carga de trabalho_ engloba uma combinação de recursos, código e serviços ou aplicações, concebida para atingir um objetivo de negócio. Isso pode ser qualquer coisa, desde um aplicativo voltado para o cliente até um processo de back-end. As cargas de trabalho podem envolver um subconjunto de recursos dentro de uma única conta da AWS ou abranger várias contas.
...
Conectividade de link Lambda:: Use a conectividade de links do Lambda para criar uma relação de confiança e conectividade entre uma conta do NetApp Workload Factory e um ou mais sistemas de arquivos Amazon FSx for ONTAP. Os links permitem que você monitore e gerencie determinados recursos do sistema de arquivos diretamente pelas chamadas da API REST do ONTAP, que não estão disponíveis por meio da API do Amazon FSx for ONTAP.
...
Operações de armazenamento EDA:: Simplifique suas operações de armazenamento de Automação de Projeto Eletrônico (EDA) no NetApp Workload Factory otimizando o desempenho, integrando com sua IaC para automatizar parâmetros de armazenamento e garantindo a integridade operacional.
...
NOTE: Os administradores da conta podem ativar ou desativar os recursos de IA. Se um administrador os desativar, o assistente de IA "Ask Me AI" não estará disponível. link:manage-genai-features.html["Saiba mais sobre como gerenciar recursos de IA"].
...
link:https://docs.netapp.com/us-en/workload-fsx-ontap/links-overview.html["Saiba mais sobre links"^]
...
link:well-architected-workloads.html["Saiba mais sobre cargas de trabalho bem arquitetadas."]
